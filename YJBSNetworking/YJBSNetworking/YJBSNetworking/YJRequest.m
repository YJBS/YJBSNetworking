...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        ...
        [YJNetworkPrivate addDoNotBackupAttribute:path];
    }
}
...
- (NSString *)cacheBasePath {
    ...
    NSArray *filters = [[YJNetworkConfig sharedInstance] cacheDirPathFilters];
    if (filters.count > 0) {
...
- (NSString *)cacheFileName {
    ...
    NSString *baseUrl = [YJNetworkConfig sharedInstance].baseUrl;
    ...
    NSString *requestInfo = [NSString stringWithFormat:@"Method:%ld Host:%@ Url:%@ Argument:%@ AppVersion:%@ Sensitive:%@",
                             ...
                             argument, [YJNetworkPrivate appVersionString], [self cacheSensitiveData]];
    NSString *cacheFileName = [YJNetworkPrivate md5StringFromString:requestInfo];
